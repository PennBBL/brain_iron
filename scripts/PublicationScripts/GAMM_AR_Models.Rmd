---
title: "Age-regressed GAMM Interaction models"
author: "Bart"
date: "5/20/2019"
output:
...
# Age and other covariates regressed out of the brain and behavior measures. The residuals are used in the models.
...
for (brain_var in c("Accumbens_Area","Putamen","Caudate","Pallidum")) {
  cat(brain_var)
  thisTab <- dataTable %>% filter(scan2cnbmonths<6) 
  g<-gamm(as.formula(sprintf("%s ~ sex + sequence + timepoint + s(age, k=4)",brain_var)),
           data=thisTab,
           random = list(bblid =~ 1))
  varname <- sprintf("%s_AR",brain_var)
  modtab <- g$gam$model %>% select(bblid,age)
  modtab[,varname] <- residuals.gam(g$gam,type = "pearson")
  thisTab <- left_join(thisTab,modtab,by = c("bblid","age"))
  
  for (beh_var in cog_vars) {
    cat(beh_var)
    
    g<-gamm(as.formula(sprintf("%s ~ sex + sequence + timepoint+ s(age, k=4)",beh_var)),
             data=thisTab,
             random = list(bblid =~ 1))
    beh_varname <- sprintf("%s_AR",beh_var)
    modtab <- g$gam$model %>% select(bblid,age)
    modtab[,beh_varname] <- residuals.gam(g$gam,type = "pearson")
    thisTab <- left_join(thisTab,modtab,by = c("bblid","age"))
  
    ar_gamm <- gamm(as.formula(sprintf("%s ~ s(%s,k=4)",varname,beh_varname)),
             data=thisTab,
             random = list(bblid =~ 1))
    cat(sprintf("\n\n### Results for %s",beh_var, brain_var))
    s_tidytable<- tidy(ar_gamm$gam)%>%
      kable(caption = sprintf("Regression table from gamm %s ~ %s",brain_var, beh_var)) %>% 
      kable_styling(full_width = F, position = "left")
    print(s_tidytable)
    b <- getViz(ar_gamm$gam)
    var_plot <- plot(b,allTerms = T) + 
      l_ciPoly() +
      l_fitRaster() + l_fitContour() + 
      l_points() +  l_ciBar() +
    ...
    ar2_gamm <- gamm(as.formula(sprintf("%s ~ s(%s,k=4) + s(%s, by = age ,k=4)",varname,beh_varname,beh_varname)),
             data=thisTab,
             random = list(bblid =~ 1))
    cat(sprintf("\n\n### Results2 for %s %s",beh_var, brain_var))
    s_tidytable<- tidy(ar_gamm$gam)%>%
      kable(caption = sprintf("Regression table from gamm %s ~ %s by age",brain_var, beh_var)) %>% 
      kable_styling(full_width = F, position = "left")
    print(s_tidytable)
    b <- getViz(ar_gamm$gam)
    var_plot <- plot(b,allTerms = T) + 
